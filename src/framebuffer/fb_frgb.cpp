...
FBFRGB::FBFRGB(std::size_t p_width, std::size_t p_height, const glm::vec3& p_clear_color, const glm::vec3& p_draw_clear_color,
           const TextureConfigurations& p_config, std::shared_ptr<Shader> p_shader, std::shared_ptr<Shader> p_draw_shader)
        : shader(p_shader), draw_shader(p_draw_shader), width(p_width), height(p_height), clear_color(p_clear_color), draw_clear_color(p_draw_clear_color), config(p_config) {
    ...
    GL_CALL(glTexImage2D, GL_TEXTURE_2D, 0, GL_RGB32F, p_width, p_height, 0, GL_RGB, GL_FLOAT, nullptr);
...
void FBFRGB::bind() const {
    GL_CALL(glBindFramebuffer, GL_FRAMEBUFFER, id);
    GL_CALL(glClearColor, clear_color.r, clear_color.g, clear_color.b, 1.0);
    GL_CALL(glViewport, 0, 0, width, height);
    GL_CALL(glClear, GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
}

void FBFRGB::bind_texture() const {
    GL_CALL(glBindTexture, GL_TEXTURE_2D, texture_id);
}

void FBFRGB::unbind() const {
    GL_CALL(glBindFramebuffer, GL_FRAMEBUFFER, 0);
}

void FBFRGB::update_size(unsigned int p_width, unsigned int p_height) {
    GL_CALL(glBindTexture, GL_TEXTURE_2D, texture_id);
    GL_CALL(glTexImage2D, GL_TEXTURE_2D, 0, GL_RGB32F, p_width, p_height, 0, GL_RGB, GL_FLOAT, nullptr);
    GL_CALL(glBindRenderbuffer, GL_RENDERBUFFER, rbo_id);
    GL_CALL(glRenderbufferStorage, GL_RENDERBUFFER, GL_DEPTH24_STENCIL8, p_width, p_height);
...
void FBFRGB::draw(struct RenderData* p_render_data) const {
    ...
    GL_CALL(glViewport, 0, 0, width, height);
    GL_CALL(glClearColor, draw_clear_color.r, draw_clear_color.g, draw_clear_color.b, 1.0);
    GL_CALL(glClear, GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
...
void FBFRGB::init_draw_data() const {
